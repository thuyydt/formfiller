...
function legacyRegexpStringParse(
  ...
  while (token != null) {
    min = Number.parseInt(token[2] ?? '0');
    max = Number.parseInt(token[3] ?? '0');
    // switch min and max
    if (min > max) {
      tmp = max;
      max = min;
  ...
  while (token != null) {
    repetitions = Number.parseInt(token[2] ?? '1');
    string =
      string.slice(0, token.index) +
      (token[1] ?? '').repeat(repetitions) +
      string.slice(token.index + token[0].length);
  ...
  while (token != null) {
    min = Number.parseInt(token[1] ?? '0'); // This time we are not capturing the char before `[]`
    max = Number.parseInt(token[2] ?? '0');
    // switch min and max
    if (min > max) {
      tmp = max;
      max = min;
 ...
 *
 * A number of methods can generate strings according to various patterns: [`replaceSymbols()`] and [`fromRegExp()`].
 */
export class HelpersModule extends SimpleHelpersModule {
  constructor(protected override readonly faker: Faker) {
    super(faker);
  }
