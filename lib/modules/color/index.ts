...
function toCSS(
  ...
  switch (cssFunction) {
    case 'rgba': {
      return `rgba(${values[0] ?? 0}, ${values[1] ?? 0}, ${values[2] ?? 0}, ${values[3] ?? 0})`;
    }

    case 'color': {
      return `color(${space} ${values[0] ?? 0} ${values[1] ?? 0} ${values[2] ?? 0})`;
    }

    case 'cmyk': {
      return `cmyk(${toPercentage(values[0] ?? 0)}%, ${toPercentage(
        values[1] ?? 0
      )}%, ${toPercentage(values[2] ?? 0)}%, ${toPercentage(values[3] ?? 0)}%)`;
    }

    case 'hsl': {
      return `hsl(${values[0] ?? 0}deg ${toPercentage(values[1] ?? 0)}% ${toPercentage(
        values[2] ?? 0
      )}%)`;
    }
    ...
    case 'hsla': {
      return `hsl(${values[0] ?? 0}deg ${toPercentage(values[1] ?? 0)}% ${toPercentage(
        values[2] ?? 0
      )}% / ${toPercentage(values[3] ?? 0)})`;
    }

    case 'hwb': {
      return `hwb(${values[0] ?? 0} ${toPercentage(values[1] ?? 0)}% ${toPercentage(
        values[2] ?? 0
      )}%)`;
    }

    case 'lab': {
      return `lab(${toPercentage(values[0] ?? 0)}% ${values[1] ?? 0} ${values[2] ?? 0})`;
    }

    case 'lch': {
      return `lch(${toPercentage(values[0] ?? 0)}% ${values[1] ?? 0} ${values[2] ?? 0})`;
    }
