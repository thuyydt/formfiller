...
export class InternetModule extends ModuleBase {
   ...
   * faker.internet.email({ firstName: 'Jeanne', lastName: 'Doe', provider: 'example.dev' })
   * faker.internet.email({ firstName: 'Jeanne', lastName: 'Doe', provider: 'example.dev', allowSpecialCharacters: true })
   *
   */
  email(
    options: {
   ...
   * faker.internet.userName({ firstName: 'Jeanne' })
   * faker.internet.userName({ firstName: 'Jeanne' })
   ...
   * faker.internet.userName({ firstName: '大羽', lastName: '陳' }) - note neither name is used
   *
   * @since 2.0.1
   *
  ...
  ipv4(
    ...
    const subnetMask = 0xffffffff >>> Number.parseInt(subnet ?? '24');
